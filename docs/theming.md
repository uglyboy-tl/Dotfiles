...
渲染逻辑由 `scripts/common/render.sh`（函数库）提供；切换实现见 `scripts/common/setting-theme.sh`。渲染产物位于 `~/.config/<app>/colors.*`，由脚本生成、不纳入版本控制（见 [架构与设计](architecture.md#生成物与版本控制)）。
...
~/.local/share/dotfiles/scripts/common/setting-theme.sh <主题名>   # 直接切换（单次）
```
...
# 创建 colors.toml 后运行
~/.local/share/dotfiles/scripts/common/setting-theme.sh my-theme
```
...
2. 在 `scripts/common/setting-theme.sh` 的 `THEME_TEMPLATES` 添加映射 `"应用:模板:输出路径"`
