...
#chsh -s $(which zsh)
sudo apt-get install --no-install-recommends fzf vim git zoxide ripgrep bat duf curl openssh-client at-spi2-core rtkit
...
sudo apt-get install --no-install-recommends file ueberzug upower xdg-desktop-portal-gtk accountsservice policykit-1-gnome

## Extra
sudo apt-get install --no-install-recommends todotxt-cli davfs2
...
#sudo apt-get install --no-install-recommends splash
