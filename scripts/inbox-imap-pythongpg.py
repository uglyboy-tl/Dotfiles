...
for email, imap_url in IMAP_DICT.items():
    completed_process = subprocess.run(['gpg', '--passphrase-file', os.path.join(os.getenv('HOME'), '.gpg/password'), '-dq', os.path.join(os.getenv('HOME'), f'.gpg/{email}.gpg')], check=True, stdout=subprocess.PIPE, encoding="utf-8")
